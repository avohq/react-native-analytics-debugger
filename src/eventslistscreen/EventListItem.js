...
export default class EventsListItem extends Component {
            ...
            <View style={[barStyles.container, {backgroundColor: backgroundColor}]}>
                ...
                <TouchableOpacity style={barStyles.rowStyle} onPress={this.rowClick}>
                    ...
                    <View style={barStyles.timestampAndNameContainer}>
                        <Text style={[barStyles.timestamp, {color: timestampColor}]}>{eventTimeString}</Text>
                        <Text style={[barStyles.eventname, {color: eventNameColor}]}>{this.props.item.name}</Text>
                    </View>
    
                    <Image style={{ height: 6, width: 10, margin: 19 }} source={rightIcon} />
                </TouchableOpacity>
    ...
    bottomView() {
        if (this.state.expended) {
            var rows = [];
            for (var eventPropIndex = 0; eventPropIndex < this.props.item.eventProps.length; eventPropIndex++) {
                rows.push(this.bottomViewRow(this.props.item.eventProps[eventPropIndex]));
                rows.push(<View key={eventPropIndex} style={baseStyles.divider} />)
            }
            for (var userPropIndex = 0; userPropIndex < this.props.item.userProps.length; userPropIndex++) {
                rows.push(this.bottomViewRow(this.props.item.userProps[userPropIndex]));
                rows.push(<View key={eventPropIndex + userPropIndex} style={baseStyles.divider} />)
            }
            rows.pop();

            return <View style={styles.expendedRowsContainer}>
                {rows}
            </View>
        } else {
            return null;
    ...
    bottomViewRow(prop) {
        ...
        return (<View key={prop.id} style={{flexDirection: 'column'}}>
            <View style={styles.expendedRow} >
                <Text style={[styles.expendedRowLabel, { color: textColor }]}>{prop.id}</Text>
                <Text style={[styles.expendedRowValue, { color: textColor }]}>{prop.value}</Text>
            </View>
            {errorMessageView}
        </View>);
    }
    ...
    boldifyErrorMessage(error) {    
        ...
        for (var allowedTypeIndex = 0; allowedTypeIndex < error.allowedTypes.length; allowedTypeIndex++) {
            boldIndexes.push(error.message.search(error.allowedTypes[allowedTypeIndex]));
        ...
        for (var boldIndex = 0; boldIndex < boldIndexes.length; boldIndex++) {
            ...
            let nonBoldPart = error.message.substring(startIndex, boldIndexes[boldIndex]);
            if (nonBoldPart.length > 0) {
                styledTextParts.push(<Text key={startIndex}>{nonBoldPart}</Text>);
            }
            styledTextParts.push(<Text key={boldIndexes[boldIndex]} style={{fontWeight:'bold'}}>{error.message.substr(boldIndexes[boldIndex], boldLengths[boldIndex])}</Text>);
        }

        if (boldIndex != 0) {
            startIndex = boldIndexes[boldIndex - 1] + boldLengths[boldIndex - 1];
        }
        styledTextParts.push(<Text key={startIndex}>{error.message.substring(startIndex)}</Text>);
